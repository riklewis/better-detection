...
define('BETTER_DETECTION_VERSION','1.7');
...
function better_detection_updated_messages($messages) {
  ...
  global $wpdb;
	...
	$type = $post->post_type;
	$from = (isset($messages[$type]) ? $type : 'post');
  $mess = " <img src='" . plugins_url('icon-36x36.png', __FILE__) . "' align='top' style='height:18px;margin:0 4px 0 18px;'>" . __('Protected by', 'better-detect-text') . " <strong>Better Detection</strong>";
	for($i=1;$i<11;$i++) {
		$curr = $messages[$from][$i];
		$messages[$type][$i] = $curr . $mess;
	}
...
function better_detection_admin_scripts() {
	if(isset($_GET["page"]) && $_GET["page"]==="better-detection-settings") {
	  ...
	  wp_enqueue_script('jquery-ui-tabs');
		...
		wp_enqueue_script('better-detection-main-js', plugins_url('main.js', __FILE__),array('jquery','jquery-ui-tabs'),BETTER_DETECTION_VERSION,true);
		wp_localize_script('better-detection-main-js', 'bsd_ajax_object', array(
...
function better_detection_show_settings() {
  ...
  echo '    <a href="https://www.php.net/supported-versions.php" target="_blank"><img src="' . better_detection_badge_php() . '"></a>';
	if(better_detection_dbtype()==='MYSQL') {
    echo ' &nbsp; <a href="https://www.fromdual.com/support-for-mysql-from-oracle" target="_blank"><img src="' . better_detection_badge_mysql() . '"></a>';
	}
	else {
		echo ' &nbsp; <a href="https://www.fromdual.com/support-for-mysql-from-oracle" target="_blank"><img src="' . better_detection_badge_maria() . '"></a>';
	}
  echo '  </div>';
